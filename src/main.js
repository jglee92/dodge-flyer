...
const ACHIEVEMENTS = [
  { score: 50, text: '🌑 운석 지대 진입!' },
  { score: 100, text: '🌌 은하 진입!' },
  { score: 180, text: '🕳️ 블랙홀 지대 진입!' },
  { score: 300, text: '☄️ 별똥별 지대 도달!' },
]
...
const ROCKET_SKINS = [
  ...
  { id: 'crimson', name: '크림슨', cost: 50, body: 0xe8e0e0, nose: 0x8b1e3f, fin: 0x5c1229 },
  { id: 'azure', name: '아주르', cost: 120, body: 0xe4eefc, nose: 0x1b6ca8, fin: 0x0f3f66 },
  { id: 'gold', name: '골드', cost: 250, body: 0xfff3c4, nose: 0xd4a017, fin: 0x8a6c0a },
  {
    id: 'neon',
    name: '🌟 네온사이버 (한정)',
    cost: 350,
  ...
  { id: 'holo', name: '✨ 홀로그램', cost: 500, body: 0xffffff, nose: 0xf0f0f0, fin: 0xe0e0e0, animated: true, sparkleCount: 2 },
  {
    id: 'diamond',
    name: '💎 다이아몬드 (한정)',
    cost: 800,
...
const FLAME_SKINS = [
  ...
  { id: 'blue', name: '블루 플레임', cost: 80, outer: 0x4fc3f7, inner: 0xd0f0ff, oscType: 'sine', freqStart: 260, freqEnd: 120 },
  { id: 'green', name: '그린 플레임', cost: 150, outer: 0x66ff99, inner: 0xe0ffe8, oscType: 'square', freqStart: 220, freqEnd: 90 },
  {
    id: 'shadow',
    name: '🌑 다크 플레임 (한정)',
    cost: 220,
...
class GameScene extends Phaser.Scene {
  ...
  renderShop() {
    ...
    const flameBonus = this.getOwnedFlamesCount() * 4
    const bonusRow = this.add
      .text(GAME_WIDTH / 2, 106, `🎁 아이템 확률 보너스   로켓 +${rocketBonus}%   불꽃 +${flameBonus}%`, {
        ...style,
        fontSize: '12px',
        align: 'center',
      })
    ...
    items.forEach((item, i) => {
      ...
      // 기본(default)은 보너스 계산에서 빠지니(getOwnedSkinsCount/getOwnedFlamesCount 참고) 태그를 안 붙인다.
      const bonusTag = owned && item.id !== 'default' ? ' (아이템 확률 +4%)' : ''
      let statusLines
  ...
  getOwnedSkinsCount() {
    return [...this.getOwnedSkins()].filter((id) => id !== 'default').length
  }

  getOwnedFlamesCount() {
    return [...this.getOwnedFlames()].filter((id) => id !== 'default').length
  }

  getCollectionBonusPercent() {
    // 특정 조합(예: 다이아몬드+다이아몬드)만 우대하면 그 조합만 노리게 되니까,
    // 대신 장착 여부와 상관없이 "지금까지 모은(구매한) 스킨 개수"에 등차적으로 걸어서
    // 어떤 스킨이든 사 모으는 것 자체가 보상이 되게 한다. 로켓/불꽃 각각 하나당 4%씩.
    const perItemPercent = 4
    return (this.getOwnedSkinsCount() + this.getOwnedFlamesCount()) * perItemPercent
  }
  ...
  update(time, delta) {
    ...
    if (this.hasShield) this.shieldRing.setPosition(this.bird.x, this.bird.y)
  }
